...
public:
    ...
    vector<vector<int>> findCriticalAndPseudoCriticalEdges(int n, vector<vector<int>>& edges) {
        int m = edges.size();
        for (int i = 0; i < m; ++i) edges[i].push_back(i); // Store the original index
        ...
        int mstWeight = 0;
        for (int i = 0; i < m; ++i) {
            if (dsu.unite(edges[i][0], edges[i][1])) mstWeight += edges[i][2];
        }
        ...
        for (int i = 0; i < m; ++i) {
            ...
            int weightWithout = 0;
            for (int j = 0; j < m; ++j) {
                if (i != j && tempDsu.unite(edges[j][0], edges[j][1])) weightWithout += edges[j][2];
            }
